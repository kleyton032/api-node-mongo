...
const criarPagamento = async (senderHash, data) => {
    try {
        ...
        else if(data.pagamento.forma === "creditCard") return  _criarPagamentoComCartao(senderHash, data);
        else return {errorMessage: "Forma de pagamento não encontrado"};
    } catch (error) {
        console.log(error);
        return {errorMessage: "Ocorreu um erro ao processar pagamento", errors: error};
    }
}
