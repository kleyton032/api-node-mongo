...
class ProdutoController {
    ...
    async index(req, res, next) {
        ...
        try {
            const produtos = await Produto.paginate({ loja: req.query.loja }, { offset, limit, sort: getSort(req.query.sortType) })
    ...
    async indexDisponiveis(req, res, next) {
        ...
        try {
            const produtos = await Produto.paginate({ loja: req.query.loja, disponibilidade: true }, { offset, limit, sort: getSort(req.query.sortType) })
            return res.send({ produtos })
        } catch (error) {
            console.log(error);
            next(error);
    ...
    async show(req, res, next){
        try {
            const produto = await Produto.findById(req.params.id).populate(["avaliacoes","Variacoes", "loja"])
            return res.send({produto})
        } catch (error) {
            console.log(error);
            next(error);
    ...
    async showAvaliacoes(req,res, next){
        try {
            const avaliacoes = await Avalicao.find({produto: req.params.id})
